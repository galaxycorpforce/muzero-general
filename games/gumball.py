...
class Gumball:
    def __init__(self):
        self.observation = numpy.zeros((1, 10), dtype="int32")
        self.player = 'p1'
        self.requester = GumballRequester()

    def to_play(self):
        return 0 if self.player == 'p1' else 1

    def reset(self):
        obs, _, _, info = self.requester.reset()
        self.observation = obs
        self.player = info['player']
        self.valid_moves = info['valid_actions']
        self.song = info['song']
        return self.get_observation()

    def step(self, action):
        obs, reward, done, info = self.requester.step(action)
        self.observation = obs
        self.player = info['player']
        self.valid_moves = info['valid_actions']
        self.song = info['song']
        done = done or len(self.legal_actions()) == 0


        return self.get_observation(), reward, done

    def get_observation(self):
#        return numpy.array([self.observation, self.song])
        return numpy.array([[self.observation]])

    def legal_actions(self):
        legal = []
        for element, idx in enumerate(self.valid_moves):
            if element == 1:
                legal.append(int(idx))
        return legal

    def expert_action(self):

        return self.requester.sample_actions()
