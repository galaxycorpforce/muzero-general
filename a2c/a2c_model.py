...
class Model(tf.keras.Model):
  def __init__(self, num_actions):
    ...
    for i in range(5):
        ...
        girl_name_embedding = Embedding(EXTRA_SMALL_VOCAB_SIZE, embedding_size, input_length=1 )
        embeddings.append(girl_name_embedding)
    ...
    self.conv1_1 = Conv1D(128, 6, activation='relu')
#    conv1 = Conv1D(100, 10, activation='relu', batch_input_shape=(None, ob_space.shape[1]))(field_inputs_)
    self.conv1_2 = Conv1D(128, 6, activation='relu')
    self.glob_1 = GlobalAveragePooling1D()
    self.drop = Dropout(0.3)


    # This returns a tensor
    non_category_data_input_keras = tf.keras.layers.Input( GUMBALL_FIELD_REMAINDER, name="non_category_data_input")
...
#    categorical_dense = Reshape(target_shape=(512,))(categorical_dense)
    ...
    self.combined_dense_2 = tf.keras.layers.Dense(1024, activation='relu')
    self.combined_dense_3 = tf.keras.layers.Dense(512, activation='relu')
